						     ...
						     void __iomem *mregs)
{
	...
	cppi->cppi_info = (struct usb_cppi41_info *)&usb_cppi41_info;;
...
static void usb_process_rx_queue(struct cppi41 *cppi, unsigned index)
{
	...
	while ((pd_addr = cppi41_queue_pop(&rx_queue_obj)) != 0) {
		...
		DBG(1, "Rx complete: dma channel(%d) ep%d len %d timeout %d\n",
		...
		if (unlikely(rx_ch->channel.actual_len >= rx_ch->length ||
			     length < orig_buf_len)) {
#if defined(CONFIG_SOC_OMAPTI81XX) || defined(CONFIG_SOC_OMAPAM33XX) || defined(CONFIG_ARCH_DAVINCI_DA8XX)
